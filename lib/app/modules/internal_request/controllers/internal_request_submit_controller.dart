...
class InternalRequestSubmitController extends GetxController{
  final List<StockoutDetail> internal_receive_medicine_list = <StockoutDetail>[].obs;

  int internalRequestNumber = 0;


  static InternalRequestSubmitController get i => Get.find();
  var dateTxtController = TextEditingController().obs;
  ...
  void addItemToList(){
    ...
    var item = ItemDispatchModel(dateTxtController.value.text,itemName.value.toString(),remark.value,int.parse(itemId.value.toString()),batchId.value,itemQty.value);
    itemList.insert(0, item);

    // if(itemList.length > 0){
    //
    //   for (final element in itemList) {
    //     if (element == itemName.value.toString()) {
    ...
    //   //   if(element.medicine_name == item.medicine_name){
    //   //     Utils.showToastAlert('${element.medicine_name} already added!');
    //   //
    //   //   }else{
    //   //
    ...
    //   itemList.insert(0, item);
    // }
    // if(dateTxtController.value.text == '' || dateTxtController.value.text == null){
    //   Utils.showToastAlert('Input date!');
    // }else if(item.medicine_name == '' || item.medicine_name == null){
    //   Utils.showToastAlert('Input item!');
    // }else if(item.medicine_qty == '' || item.medicine_qty == null){
    //   Utils.showToastAlert('Input quantity!');
    // }else{
    //   if(itemList.length > 0){
    //     itemList.forEach((element) {
    //       if(element.medicine_name == item.medicine_name){
